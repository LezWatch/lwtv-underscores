...
class CPTs implements Component, Templater {
	...
	public function get_tmdb_info( $post_id ): mixed {
		...
		if ( ! defined( 'TMDB_API' ) ) {
			lwtv_plugin()->debug_log( 'tmdb', 'TMDB API not defined' );
			return false;
		}
		...
		if ( ! $post_type ) {
			lwtv_plugin()->debug_log( 'tmdb', 'Invalid post type got TMDB info: ' . get_post_type( $post_id ) );
			return false;
		}
		...
		if ( ! $tmdb_id && ! $imdb_id ) {
			lwtv_plugin()->debug_log( 'tmdb', 'No TMDB or IMDB ID found for post ID: ' . $post_id );
			return false;
		}
		...
		try {
			...
			if ( isset( $body['status_message'] ) ) {
				lwtv_plugin()->debug_log( 'tmdb', 'Error getting TMDB info: ' . $body['status_message'] );
				return false;
			}
