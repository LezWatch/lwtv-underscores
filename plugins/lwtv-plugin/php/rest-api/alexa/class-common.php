<?php
/**
 * REST-API - Alexa Skills - Common/Shared Tools
 *
 * There are some things that are used by multiple skills. In order to
...
class Common {
	...
	public function bury_your_queers_rest_api_callback( \WP_REST_Request $request ) {
		...
		// Call the validation:
		$validate_alexa = ( new Validate() )->the_request( $request );
			...
			switch ( $intent ) {
				...
				case 'HowMany':
					if ( false === $date || false === $timestamp ) {
						...
						$data     = ( new Stats_JSON() )->statistics( 'death', 'years' );
