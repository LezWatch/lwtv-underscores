...
class Related_Posts {
	 ...
	 * @return string
	 */
	public function related_posts( $slug, int $max_posts = 3, string $format = '' ): mixed {

		$the_related_posts = null;
		...
		if ( isset( $related_post_query ) && is_array( $related_post_query ) ) {
			$the_related_posts = $this->related_display( $related_post_query, $slug, $max_posts, $format );
		}
	 ...
	 *
	 * @param  array  $related_post_query
	 * @param  string $slug
	 * @param  int    $max_posts
	 * @param  string $format
	 * @return mixed
	 */
	public function related_display( array $related_post_query, mixed $slug, int $max_posts = 3, string $format = '' ): mixed {
		// Default content:
		...
		if ( 'overlay' === $format ) {
			$the_related_posts = array(
				'total' => count( $related_post_query ),
				'posts' => array(),
			);
			foreach ( $related_post_query as $related_post ) {
				if ( $count_post <= $max_posts ) {
					$the_related_posts['posts'][] = $related_post;
					++$count_post;
				}
			}
		} else {
			...
			foreach ( $related_post_query as $related_post ) {
				if ( $count_post <= $max_posts ) {
					...
					<div class="related-article-image-wrapper"><a href="' . get_the_permalink( $related_post ) . '">' . get_the_post_thumbnail( $related_post, 'medium' ) . '</a></div>
					<div class="card-body">
						<span class="card-title related-article-header"><a href="' . get_the_permalink( $related_post ) . '">' . get_the_title( $related_post ) . '</a></span>
	...
	public function related_archive_header( $tag_id ): string {
		...
		$icons       = array(
			'show'      => lwtv_plugin()->get_symbolicon( svg: 'tv-hd.svg', icon: 'svg-tv' ),
			'actor'     => lwtv_plugin()->get_symbolicon( svg: 'team.svg', icon: 'svg-users' ),
			'character' => lwtv_plugin()->get_symbolicon( svg: 'contact-card.svg', icon: 'svg-users' ),
		);
