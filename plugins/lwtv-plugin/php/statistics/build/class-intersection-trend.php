...
class Intersection_Trend {

	/**
	 * Decade-bucketed intersection mix, oldest to newest.
	 *
	...
	public function generate( int $min_bucket_size = 20, int $top_n = 3 ): array {
		return Genre_Decade_Buckets::build( $this->get_year_intersection_tally(), $min_bucket_size, $top_n );
	}

	/**
	 * One SQL pass: every published show's premiere-year meta paired with
	 * each lez_intersections term it carries (one row per show-term pair,
	 * so a show with several intersections contributes several rows).
	 * Rolled up into
	 * [ year => { 'shows': int, 'genres': [ slug => { 'name': string, 'count': int }, … ] } ]
	 * — the 'genres' key name is Genre_Decade_Buckets' generic input shape,
	 * not a mislabel. 'shows' is deduped per show ID so a show with three
	 * intersections still only counts once toward that year's total.
	 *
	 * @return array
	 */
	private function get_year_intersection_tally(): array {
		global $wpdb;

		$cache_key   = 'intersection_trend_year_tally';
		...
		try {
			$query = "SELECT
				...
				INNER JOIN {$wpdb->term_taxonomy} tt ON tr.term_taxonomy_id = tt.term_taxonomy_id AND tt.taxonomy = 'lez_intersections'
			...
			if ( ! is_array( $results ) ) {
				lwtv_plugin()->debug_log( 'statistics', 'Intersection trend query failed: ' . $wpdb->last_error );
				return array();
			}

			// Two running totals per year: which show IDs have been seen
			// (deduped, since one show can produce several rows above) and
			// the raw per-term tag counts (intentionally NOT deduped — each
			// intersection a show carries should count toward that term).
			...
			lwtv_plugin()->error_log( 'statistics', 'Error generating intersection trend statistics: ' . $e->getMessage() );
			return array();
		}
	}
}
