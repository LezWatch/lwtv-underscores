...
class Dead_Shows {
	...
	public function make( $format ) {
		try {
			...
			lwtv_plugin()->error_log( 'dead-shows-error', 'Error in dead shows make method: ' . $e->getMessage() );
	...
	private function build_dead_shows_optimized( $format ) {
		...
		try {
			...
			lwtv_plugin()->error_log( 'dead-shows-error', 'Error building dead shows statistics: ' . $e->getMessage() );
