...
class Stats_Generator {
	...
	public function generate_shows( $format = 'list', $type = 'formats' ) {
		...
		if ( empty( $all_data ) ) {
			lwtv_plugin()->error_log( 'statistics', 'All data for shows is empty' );
			return array();
		}
	...
	public function generate_characters( $format = 'list', $type = 'cliches' ) {
		...
		if ( empty( $all_data ) ) {
			lwtv_plugin()->error_log( 'statistics', 'All data for characters is empty' );
			return array();
		}
	...
	public function generate_actors( $format = 'list', $type = 'gender' ) {
		...
		if ( empty( $all_data ) ) {
			lwtv_plugin()->error_log( 'statistics', 'All data for actors is empty' );
			return array();
		}
	...
	public function generate_dead( $subject, $view, $format ) {
		...
		if ( 'count' === $format ) {
			if ( ! in_array( $subject, array( 'characters', 'shows' ), true ) ) {
				lwtv_plugin()->error_log( 'statistics', 'Invalid subject for death count: ' . $subject );
				return 0;
			}
		...
		if ( empty( $all_data ) ) {
			lwtv_plugin()->error_log( 'statistics', 'All data for dead is empty' );
			return array();
		}
	...
	public function generate_individual_actors( $actor_id, $format = 'piechart', $type = 'roles' ) {
		...
		switch ( $type ) {
			case 'roles':
				$view = 'roles';
				lwtv_plugin()->error_log( 'statistics', 'Generating ROLES statistics for actor: ' . $actor_id );
				$all_data[ $view ] = ( new Build_Actors() )->generate_roles( $actor_id );
				break;
			case 'dead':
				$view = 'dead';
				lwtv_plugin()->error_log( 'statistics', 'Generating DEAD statistics for actor: ' . $actor_id );
