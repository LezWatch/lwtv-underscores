...
<div class="lwtv-toll">
	...
	foreach ( $death_cards as $death_card ) {
		?>
		<div class="lwtv-toll-tile lwtv-toll-tile--<?php echo esc_attr( $death_card['variant'] ); ?>">
			<div class="lwtv-toll-top">
				<div>
					<span class="lwtv-toll-eyebrow"><?php echo esc_html( $death_card['label'] ); ?></span>
					<span class="lwtv-toll-num" data-count-to="<?php echo (int) $death_card['count']; ?>" data-count-suffix="<?php echo esc_attr( $death_card['suffix'] ); ?>"><?php echo esc_html( number_format_i18n( $death_card['count'] ) . $death_card['suffix'] ); ?></span>
				</div>
				<span class="lwtv-toll-chip"><?php echo lwtv_plugin()->get_symbolicon( svg: $death_card['svg'], icon: $death_card['icon'], max_size: '19' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
			</div>
			<span class="lwtv-toll-caption"><?php echo esc_html( $death_card['caption'] ); ?></span>
		</div>
		<?php
	}
	?>
...
// Most recent year that actually has a death (the current year may be 0 so far,
// which would read as a data gap rather than a toll).
$death_latest = array(
	'year'  => 0,
	'count' => 0,
);
foreach ( array_reverse( $death_ys['rows'] ) as $death_row ) {
	if ( (int) $death_row['count'] > 0 ) {
		$death_latest = $death_row;
		break;
	}
}
