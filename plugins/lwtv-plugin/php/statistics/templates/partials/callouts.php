	...
	<?php foreach ( $lwtv_callouts as $lwtv_callout ) : ?>
		<div class="lwtv-trend-callout">
			...
			<?php if ( ! empty( $lwtv_callout['icon'] ) ) : ?>
				<span class="lwtv-trend-callout-icon"><?php echo lwtv_plugin()->get_symbolicon( svg: $lwtv_callout['icon'], icon: 'svg-' . str_replace( '.svg', '', $lwtv_callout['icon'] ), max_size: '24' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
			<?php endif; ?>
		</div>
	<?php endforeach; ?>
</div>
