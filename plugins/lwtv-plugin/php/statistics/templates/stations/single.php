		...
		if ( Overview_Factsheet::collapse_for_shows( $lwtv_shows ) ) {
			...
			if ( $lwtv_sh_first > 0 && $lwtv_sh_first === $lwtv_sh_last ) {
				$lwtv_sh_lines[] = sprintf(
					/* translators: 1: station name, 2: the year on air, 3: number of shows. */
				...
				$lwtv_sh_lines[] = sprintf(
					/* translators: 1: station name, 2: first year on air, 3: last year on air, 4: number of shows. */
				...
				$lwtv_sh_lines[] = sprintf(
					/* translators: 1: station name, 2: number of shows. */
			...
			$lwtv_oa_desc = __( 'Shows on air in each year, starting from the first tracked episode.', 'lwtv' );
			if ( '' !== $lwtv_gap_txt ) {
