...
class Monitors {
	...
	public function init() {
		add_submenu_page( 'lwtv', 'Monitor Status', 'Monitor Status', 'upload_files', 'lwtv_monitor_check', array( $this, 'settings_page' ) );
	}
	...
	public static function check_tvmaze() {
		...
		$return = array(
			...
			'icon'        => lwtv_plugin()->get_symbolicon( $icon['symbolicon'], $icon['font-awesome'], 'symbolicons ' . $icon['class'] ),
			'status'      => $status,
		);
	...
	public static function check_gtmetrix() {
		...
		if ( ! defined( 'LWTV_GTMETRIX' ) ) {
			...
			$status = lwtv_plugin()->get_transient( 'lwtv_gtmetrix' );
		...
		$grade = lwtv_plugin()->get_transient( 'lwtv_gtmetrix_grade' );
		if ( isset( $grade ) ) {
			switch ( $grade ) {
				case 'A':
					$icon = array(
