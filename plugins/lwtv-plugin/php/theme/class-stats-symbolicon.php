...
class Stats_Symbolicon {
	...
	public function make( $stats_type ) {
		...
		// Defaults:
		$stats_array = array(
			'image' => lwtv_plugin()->get_symbolicon( svg: 'graph-bar.svg', fontawesome: 'fa-chart-area' ),
			'intro' => '',
		);
		...
		switch ( $stats_type ) {
			case 'death':
				$stats_array['image'] = lwtv_plugin()->get_symbolicon( svg: 'grim-reaper.svg', fontawesome: 'fa-ban' );
				...
				break;
			case 'characters':
				$stats_array['image'] = lwtv_plugin()->get_symbolicon( svg: 'chart-bar.svg', fontawesome: 'fa-chart-bar' );
				$stats_array['intro'] = 'Data specific to queer characters.';
				break;
			case 'actors':
				$stats_array['image'] = lwtv_plugin()->get_symbolicon( svg: 'graph-line.svg', fontawesome: 'fa-chart-line' );
				$stats_array['intro'] = 'Data specific to actors who play queer characters.';
				break;
			case 'shows':
				$stats_array['image'] = lwtv_plugin()->get_symbolicon( svg: 'chart-pie.svg', fontawesome: 'fa-chart-pie' );
				$stats_array['intro'] = 'Data specific to TV shows with queer characters.';
				break;
			case 'nations':
				$stats_array['image'] = lwtv_plugin()->get_symbolicon( svg: 'globe.svg', fontawesome: 'fa-globe' );
				$stats_array['intro'] = 'Data specific to queer representation on shows by nation.';
				break;
			case 'stations':
				$stats_array['image'] = lwtv_plugin()->get_symbolicon( svg: 'satellite-signal.svg', fontawesome: 'fa-bullhorn' );
