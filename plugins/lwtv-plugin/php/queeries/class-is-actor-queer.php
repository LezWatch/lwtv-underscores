	...
	private function check_queerness( $taxonomies ): bool {
		...
		if ( ! empty( $gender_terms ) ) {
			...
			if ( ! empty( $non_straight_genders ) ) {
				lwtv_plugin()->error_log( 'is-queer', 'Actor is queer based on gender terms: ' . wp_json_encode( $gender_terms ) );
				return true;
			}
		}
		...
		if ( ! empty( $pronoun_terms ) && ! empty( $gender_terms ) ) {
			...
			if ( ! empty( $straight_pronouns ) ) {
				...
				if ( $has_non_straight_pronouns ) {
					lwtv_plugin()->error_log( 'is-queer', 'Actor is queer based on pronouns: ' . wp_json_encode( $pronoun_terms ) . ' for gender: ' . wp_json_encode( $gender_terms ) );
					return true;
				}
			} else {
				...
				lwtv_plugin()->error_log( 'is-queer', 'Actor is queer based on pronouns (no straight definition): ' . wp_json_encode( $pronoun_terms ) . ' for gender: ' . wp_json_encode( $gender_terms ) );
				return true;
			}
		}
		...
		if ( ! empty( $sexuality_terms ) ) {
			...
			if ( ! empty( $non_straight_sexuality ) ) {
				lwtv_plugin()->error_log( 'is-queer', 'Actor is queer based on sexuality terms: ' . wp_json_encode( $sexuality_terms ) );
				return true;
			}
		}
		...
		if ( ! empty( $romantic_terms ) ) {
			...
			if ( ! empty( $non_straight_romantic ) ) {
				lwtv_plugin()->error_log( 'is-queer', 'Actor is queer based on romantic terms: ' . wp_json_encode( $romantic_terms ) );
				return true;
			}
		}
