<?php
/*
 * Validation: BYQ Checks For LezWatch.TV
 *
 */
...
use LWTV\Debugger\Characters as Character_Debugger;
...
class BYQ_Checker {
	...
	public static function make() {
		...
		if ( ( isset( $_POST['rerun'] ) && check_admin_referer( 'run_byq_checker_clicked' ) ) || false === $items ) {
			$items = ( new Character_Debugger() )->find_byq_problems();
		}

		// If recheck was clicked, only check the problem children.
		if ( isset( $_POST['recheck'] ) && check_admin_referer( 'run_byq_checker_clicked' ) && false !== $items ) {
			$items = ( new Character_Debugger() )->find_byq_problems( $items );
		}

		// Get the last run time.
		$last_run = ( new Validation() )->last_run( 'byq_problems' );
		...
		if ( empty( $items ) || ! is_array( $items ) ) {
			...
			<div class="lwtv-tools-container lwtv-tools-container__alert">
				...
				<div id="lwtv-tools-alerts">
					<p>All the death data looks good and the data looks sane.</p>
			...
			<div class="lwtv-tools-container lwtv-tools-container__alert">
				...
				<div id="lwtv-tools-alerts">
					...
					<p>If a character was on multiple shows, the show on which they died is the <strong>only</strong> one where they should be marked as dead!</p>
					<?php echo wp_kses_post( $last_run ); ?>
				</div>
			</div>
		...
		?>
		<form action="admin.php?page=lwtv_data_check&tab=tab_byq_checker" method="post">
			<?php wp_nonce_field( 'run_byq_checker_clicked' ); ?>
