...
class TMDB_Batch_Task {
	...
	public function queue_post( int $post_id ): bool {
		...
		if ( ! $post_type ) {
			lwtv_plugin()->error_log( 'tmdb-batch', "Invalid post ID: {$post_id}" );
			return false;
		}

		// Check if already has TMDB ID
		if ( $this->has_tmdb_id( $post_id, $post_type ) ) {
			lwtv_plugin()->error_log( 'tmdb-batch', "Post {$post_id} already has TMDB ID" );
			return false;
		}
		...
		lwtv_plugin()->error_log( 'tmdb-batch', "Queued post {$post_id} for TMDB processing" );
		return true;
	}
	...
	public function process_tmdb_batch(): void {
		...
		if ( empty( $queued_posts ) ) {
			lwtv_plugin()->error_log( 'tmdb-batch', 'No posts queued for TMDB processing' );
			return;
		}

		lwtv_plugin()->error_log( 'tmdb-batch', 'Processing ' . count( $queued_posts ) . ' posts for TMDB data' );
		...
		foreach ( $batches as $batch_index => $batch ) {
			lwtv_plugin()->error_log( 'tmdb-batch', 'Processing batch ' . ( $batch_index + 1 ) . ' of ' . count( $batches ) );
		...
		lwtv_plugin()->error_log( 'tmdb-batch', "Completed TMDB batch processing: {$success_count}/{$processed_count} successful" );
	...
	private function process_batch( array $batch ): array {
		...
		foreach ( $batch as $post_data ) {
			...
			if ( $this->is_rate_limited() ) {
				lwtv_plugin()->error_log( 'tmdb-batch', 'Rate limit hit, pausing batch processing' );
				$rate_limit_hit = true;
				break;
			}
	...
	private function process_single_post( array $post_data ): bool {
		...
		try {
			...
			if ( ! $tmdb_data ) {
				lwtv_plugin()->error_log( 'tmdb-batch', "No TMDB data found for post {$post_id}" );
				return false;
			}
			...
			if ( $tmdb_id ) {
				...
				lwtv_plugin()->error_log( 'tmdb-batch', "Successfully saved TMDB ID {$tmdb_id} for post {$post_id}" );
				return true;
			} else {
				lwtv_plugin()->error_log( 'tmdb-batch', "No TMDB ID found in data for post {$post_id}" );
				return false;
			}
		} catch ( \Exception $e ) {
			lwtv_plugin()->error_log( 'tmdb-batch', "Error processing post {$post_id}: " . $e->getMessage() );
			return false;
		}
	}
