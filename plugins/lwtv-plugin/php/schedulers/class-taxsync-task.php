...
class Taxsync_Task {
	...
	public function process_taxsync_task( int $post_id ): void {
		...
		if ( ! $post_type ) {
			lwtv_plugin()->error_log( 'taxsync', "Invalid post ID: {$post_id}" );
			return;
		}
		...
		if ( ! $post || 'publish' !== $post->post_status ) {
			lwtv_plugin()->error_log( 'taxsync', "Skipping taxonomy sync for {$post_type} ID: {$post_id} - post not published or doesn't exist" );
			return;
		}

		lwtv_plugin()->error_log( 'taxsync', "Processing taxonomy sync task for {$post_type} ID: {$post_id}" );
		...
		try {
			...
			switch ( $post_type ) {
				...
				case 'post_type_characters':
					...
					break;
				default:
					lwtv_plugin()->error_log( 'taxsync', "Unsupported post type: {$post_type} for ID: {$post_id}" );
					return;
			}

			if ( $success ) {
				lwtv_plugin()->error_log( 'taxsync', "Successfully completed taxonomy sync for {$post_type} ID: {$post_id}" );
			} else {
	...
	private function process_actor_taxonomy_sync( int $post_id ): bool {
		lwtv_plugin()->error_log( 'taxsync', "Processing actor taxonomy sync for ID: {$post_id}" );

		// Get actor taxonomy mappings from the Actors class
		$taxonomy_mappings = Actors::SELECT2_TAXONOMIES;

		if ( empty( $taxonomy_mappings ) ) {
			lwtv_plugin()->error_log( 'taxsync', "No taxonomy mappings found for actor ID: {$post_id}" );
		...
		foreach ( $taxonomy_mappings as $postmeta => $taxonomy ) {
			try {
				...
				++$success_count;
				lwtv_plugin()->error_log( 'taxsync', "Synced taxonomy {$taxonomy} for actor ID: {$post_id}" );
			} catch ( \Exception $e ) {
		...
		lwtv_plugin()->error_log( 'taxsync', "Completed actor taxonomy sync for ID: {$post_id} - {$success_count}/{$total_count} successful" );
	...
	private function process_show_taxonomy_sync( int $post_id ): bool {
		lwtv_plugin()->error_log( 'taxsync', "Processing show taxonomy sync for ID: {$post_id}" );

		// Get show taxonomy mappings from the Shows class
		$taxonomy_mappings = Shows::SELECT2_TAXONOMIES;

		if ( empty( $taxonomy_mappings ) ) {
			lwtv_plugin()->error_log( 'taxsync', "No taxonomy mappings found for show ID: {$post_id}" );
			return true; // No mappings to sync, consider it successful
		}
	...
	private function process_character_taxonomy_sync( int $post_id ): bool {
		lwtv_plugin()->error_log( 'taxsync', "Processing character taxonomy sync for ID: {$post_id}" );

		// Character taxonomy mappings
		$taxonomy_mappings = Characters::SELECT2_TAXONOMIES;

		if ( empty( $taxonomy_mappings ) ) {
			lwtv_plugin()->error_log( 'taxsync', "No taxonomy mappings found for character ID: {$post_id}" );
			return true;
		}
	...
	private function handle_taxsync_failure( int $post_id, string $post_type, string $error_msg ): void {
		...
		lwtv_plugin()->error_log( 'taxsync', "Taxonomy sync failed for {$post_type} ID: {$post_id} - {$error_msg} (attempt {$retry_count}/{$max_retries})" );
		...
		if ( $retry_count < $max_retries ) {
			...
			lwtv_plugin()->error_log( 'taxsync', "Scheduled retry {$retry_count} for {$post_type} ID: {$post_id} in {$delay} seconds" );
		} else {
			// Max retries reached, log final failure
			lwtv_plugin()->error_log( 'taxsync', "Taxonomy sync permanently failed for {$post_type} ID: {$post_id} after {$max_retries} attempts" );
