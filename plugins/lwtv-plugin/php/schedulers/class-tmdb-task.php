...
class TMDB_Task {
	...
	public function process_tmdb_task( int $post_id ): void {
		...
		lwtv_plugin()->error_log( 'tmdb', "Processing TMDB task for {$post_type} ID: {$post_id}" );

		// Check if TMDB ID is already set
		$tmdb_id = $this->get_existing_tmdb_id( $post_id, $post_type );
		if ( $tmdb_id ) {
			lwtv_plugin()->error_log( 'tmdb', "TMDB ID already exists for {$post_type} ID: {$post_id}" );
			return;
		}
		...
		if ( ! $tmdb_data ) {
			lwtv_plugin()->error_log( 'tmdb', "No TMDB data found for {$post_type} ID: {$post_id}" );
			return;
		}
		...
		if ( $tmdb_id ) {
			...
			lwtv_plugin()->error_log( 'tmdb', "Successfully saved TMDB ID: {$tmdb_id} for {$post_type} ID: {$post_id}" );
		} else {
			lwtv_plugin()->error_log( 'tmdb', "No TMDB ID found in data for {$post_type} ID: {$post_id}" );
		}
	}
