...
class Cache_Task {
	...
	public function process_cache_task( int $post_id ): void {
		...
		if ( ! $post_type ) {
			lwtv_plugin()->error_log( 'caching', "Invalid post ID: {$post_id}" );
			return;
		}

		lwtv_plugin()->error_log( 'caching', "Processing cache task for {$post_type} ID: {$post_id}" );
		...
		if ( isset( $clear_urls ) && ! empty( $clear_urls ) ) {
			...
			lwtv_plugin()->error_log( 'caching', "Successfully cleared cache for {$post_type} ID: {$post_id} - {$clear_url_count} URLs" );
		} else {
			lwtv_plugin()->error_log( 'caching', "No cache URLs to clear for {$post_type} ID: {$post_id}" );
		}
	}
}
