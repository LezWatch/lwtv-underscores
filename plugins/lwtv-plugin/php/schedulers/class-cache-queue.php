...
class Cache_Queue {
	...
	public static function queue( int $post_id ): void {
		...
		lwtv_plugin()->error_log( 'caching', "Queued cache invalidation for post ID: {$post_id}" );
	}
	...
	public function process_on_shutdown(): void {
		...
		lwtv_plugin()->error_log( 'caching', 'Processing cache queue on shutdown - ' . count( self::$post_ids_to_process ) . ' posts, ' . count( self::$urls_to_clear ) . ' URLs' );
		...
		if ( ! empty( $unique_urls ) ) {
			...
			lwtv_plugin()->error_log( 'caching', 'Successfully cleared cache for ' . count( $unique_urls ) . ' URLs' );
		}
