...
class Cache_Batch_Task {
	...
	public function queue_post( int $post_id ): bool {
		...
		if ( ! $this->is_processing_scheduled() ) {
			...
			lwtv_plugin()->error_log( 'cache-batch', 'Scheduled cache batch processing' );
		}
	...
	public function process_cache_batch(): void {
		if ( ! lwtv_plugin()->is_action_scheduler_available() ) {
			lwtv_plugin()->error_log( 'cache-batch', 'Action Scheduler not available for cache batch processing' );
			return;
		}
		...
		if ( empty( $queue ) ) {
			lwtv_plugin()->error_log( 'cache-batch', 'Cache batch queue is empty' );
			$this->clear_status();
			return;
		}
		...
		if ( ! empty( $unique_urls ) ) {
			...
			foreach ( $url_batches as $batch_index => $url_batch ) {
				...
				lwtv_plugin()->error_log( 'cache-batch', 'Processed URL batch ' . ( $batch_index + 1 ) . ' of ' . count( $url_batches ) . ' - ' . count( $url_batch ) . ' URLs' );
		...
		if ( ! empty( $remaining_queue ) ) {
			...
			lwtv_plugin()->error_log( 'cache-batch', 'Scheduled next cache batch processing - ' . count( $remaining_queue ) . ' posts remaining' );
		} else {
			$this->clear_status();
			lwtv_plugin()->error_log( 'cache-batch', 'Cache batch processing completed' );
		}
	}
	...
	private function process_url_batch( array $urls ): bool {
		try {
			...
			lwtv_plugin()->error_log( 'cache-batch', 'Error processing URL batch: ' . $e->getMessage() );
			return false;
		}
	}
	...
	public function clear_queue(): bool {
		...
		lwtv_plugin()->error_log( 'cache-batch', 'Cache batch queue cleared' );
		return true;
	}
	...
	public function trigger_processing(): bool {
		...
		lwtv_plugin()->error_log( 'cache-batch', 'Triggered immediate cache batch processing' );
		return true;
	}
}
