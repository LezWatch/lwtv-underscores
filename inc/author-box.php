...
function lwtv_author_box( $content ) {
	...
	if ( is_singular( 'post' ) && isset( $post->post_author ) ) {
		...
		$author_details .= '<div class="author-archives">' . lwtv_symbolicons( 'newspaper.svg', 'fa-newspaper' ) . '&nbsp;<a href="' . $user_posts . '">' . $user_articles . '</a></div>';

		// Add Twitter if it's there
		$author_details .= ( ! empty( $user_twitter ) ) ? '<div class="author-twitter">' . lwtv_symbolicons( 'twitter.svg', 'fa-twitter' ) . '&nbsp;<a href="https://twitter.com/' . $user_twitter . '" target="_blank" rel="nofollow">@' . $user_twitter . '</a> </div>' : '';

		// Add favourite shows if they're there
		$author_details .= ( isset( $favourites ) && ! empty( $favourites ) ) ? '<div class="author-favourites">' . lwtv_symbolicons( 'tv-hd.svg', 'fa-tv' ) . '&nbsp;Favorite ' . $fav_title . ': ' . $favourites . '</div>' : '';
