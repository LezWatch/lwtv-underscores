jQuery(function($) {
	const overlayList = document.querySelectorAll('div[data-modal-type="overlay"]')
	...
	if ( currentView ) {
		var showModal = new bootstrap.Modal(document.getElementById( currentView ), {})
		showModal.toggle();
	}
	...
	overlayList.forEach(overlayListEl => {
		...
		overlayListEl.addEventListener( 'show.bs.modal', (e) => {
			var nextTab  = e.target.getAttribute('id').replace( '-modal', '' );
