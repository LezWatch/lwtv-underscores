# Plan: Automating the LWTV On-Air Show & Character Audit

Replaces the manual quad-monthly audit with `wp lwtv audit`, outputting a CSV of only the entries that need human attention.

## Command surface

Two modes, following the existing positional-arg pattern (`cli-generate.php`, `cli-sweep.php`):

```
wp lwtv audit shows [--letter=<a-z|num|intl>] [--format=<table|csv|json>]
wp lwtv audit show <id> [--recurring] [--guests] [--all] [--format=<table|csv|json>]
```

**`audit shows`** is the quad-monthly catalog audit: show status vs TVMaze, plus living *regular* characters missing the current year. Cheap (2 API calls per show), heuristic, actor-independent.

**`audit show <id>`** is the deep audit of one show: per-episode guest-cast checking against TVMaze. This is where `--recurring` and `--guests` live, and `--all` extends the check to the show's full history — the "we just added this show and need a historical pass" case. Expensive, name-matching based, produces *verify* rows rather than certainties.

There is intentionally no catalog-wide `--guests`. If that's ever wanted, it's an Action Scheduler batch job, not a CLI run.

## Data sources (actual schema)

| Purpose | Source |
|---|---|
| On-air shows | `lezshows_on_air` meta = `yes` (maintained by Shows calculations + OnAir debugger) |
| Show CPT slug | `post_type_shows` (`CPT_Shows::SLUG`) |
| TVMaze lookup | `LWTV\Calendar\TVMaze::get_tvmaze_info_show()` — cached `lezshows_tvmaze_id` first, `lezshows_imdb` fallback, name search last |
| Characters on a show | `lezshows_char_list` meta, with REGEXP sub-field query fallback (pattern from `class-shows-builder.php`) |
| Character↔show data | ACF repeater `lezchars_show_group`: `show` (post ID), `type` (`regular`/`recurring`/`guest`), `appears` (array of years) |
| Dead filter | `dead` term in `lez_cliches` |
| **Animation filter** | `animation` or `anime` terms in `lez_genres` — **character auditing is skipped entirely** for these shows. Voice actors play multiple characters (Simpsons, South Park), so TVMaze cast data can't be mapped to LWTV characters; those stay manual via fan wikis. Show *status* is still checked. |
| Actor name | First ID in `lezchars_actor` |
| Letter buckets | `Shared_Builder::get_character_marker()` — `#` numbers, `A–Z` ASCII, `-` everything else (Thai, accents, symbols) |

## `--letter` design

Buckets must match `Shared_Builder::get_character_marker()` exactly so the audit agrees with the site's alphabet pages. That means **no SQL title filtering**: `LIKE 'a%'` under utf8mb4 collations is accent-insensitive (`Á…` would land in `a` while the site files it under `-`). Instead, the on-air query returns only IDs (a few hundred), and we filter in PHP with the existing method.

Flag values, since `#` starts a shell comment and bare `-` fights argument parsing:

- `--letter=a` … `--letter=z`
- `--letter=num` → the `#` bucket (quoted `--letter='#'` also accepted)
- `--letter=intl` → the `-` bucket (quoted `--letter='-'` also accepted)

Known and accepted: buckets follow the raw title, so every "The …" show lands in T. Big T. Consistency with the site wins.

## Catalog audit logic (`audit shows`)

**Show-level rows** (all shows, including animated):

- TVMaze status `Ended` → "Set end year" (with TVMaze's `ended` date)
- TVMaze status `To Be Determined` → "Review: show in limbo"
- No TVMaze match → "Add IMDb/TVMaze ID or audit manually"

**Character-level rows** (only when the show is not animated, is `Running`, *and* aired an episode this year per the previous-episode link):

- Living `regular` character without the current year in `appears` for this show → "Add {year} to Years Appears"

Dead characters are skipped. Between-seasons shows produce no character rows. Animated shows produce a single informational row noting characters were skipped, so the CSV is honest about coverage.

## Deep audit logic (`audit show <id>`)

Refuses animated shows outright with an explanation (same reasoning as above — there's nothing useful it could output).

1. Resolve the show on TVMaze (same class, same retry).
2. Fetch the all-time main cast (`/shows/:id/cast`, 1 call) and the full episode list (`/shows/:id/episodes`, 1 call).
3. Determine audit years: current year only, or every year with episodes when `--all`.
4. For each audit year, build a set of normalized names (person + character) seen that year: main-cast names count for every year (TVMaze doesn't date cast membership), plus `/episodes/:id/guestcast` for each episode in that year.
5. Roles audited: `regular` always; `recurring` with `--recurring`; `guest` with `--guests`.
6. For each matching character row on this show, per year:
   - Name found in year Y, Y **not** in `appears` → "TVMaze shows {year} — add?"
   - Y in `appears`, name **not** found in year Y → "Verify {year} — no TVMaze appearance found"

**Dead characters:** skipped for current-year-only runs (same noise argument as the catalog audit), but **included** when `--all` is set — a dead character's historical `appears` years are exactly what a historical pass should validate. *(Flagging this as a judgment call — easy to flip.)*

### Guest-cast caching (what makes this affordable)

Aired episodes are immutable, so compiled name-sets are cached per show+year via the `lwtv_plugin()` transient wrappers:

- Past years: `YEAR_IN_SECONDS` (effectively forever)
- Current year: `DAY_IN_SECONDS` (new episodes keep landing)

First run on a show pays full price; every run after only fetches uncached years plus a refresh of the current one. When the uncached call count exceeds 300, the command shows the estimate and asks for confirmation (`--yes` skips it, per normal WP-CLI behavior).

### Matching caveats (why output says "verify", not "fix")

Matching is normalized — `remove_accents()`, parentheticals stripped (LWTV disambiguation titles like "Sara Lance (Legends of Tomorrow)"), case-insensitive — against both character and person names. It will still miss on "Dr." vs "Doctor" style variance, and old episodes often have sparse guestcast data on TVMaze, which inflates "Verify" rows for early seasons. These are review lists. There is no `--fix` flag on purpose.

## API budget

| Run | Calls | Wall time |
|---|---|---|
| `audit shows` (catalog, ~300 shows) | ~500–600 | 5–8 min |
| `audit shows --letter=a` | proportional slice | ~1 min |
| `audit show <id>` (scripted, current year) | ~10–25 | seconds |
| `audit show <id> --all` (scripted, 5 seasons) | ~100 | ~1 min |
| `audit show <id> --all` (a soap, first run) | **10,000–15,000** | **hours** |
| Any repeat run on cached years | near zero | seconds |

Rate limit is ~20 calls/10s with no auth; the command spaces calls at 0.5s and honors `Retry-After` on 429 (max 3 attempts).

## The command

New file: `php/wp-cli/cli-audit.php`
...
class WP_CLI_LWTV_Audit {
	...
	private function audit_single( int $show_id, array $assoc_args ): void {
		...
		$cast_names = array();
		$cast       = $this->tvmaze_get( 'https://api.tvmaze.com/shows/' . $tvmaze_id . '/cast' );
		if ( is_array( $cast ) ) {
		...
		$episodes = $this->tvmaze_get( 'https://api.tvmaze.com/shows/' . $tvmaze_id . '/episodes' );
	...
	private function get_year_names( int $show_id, int $year, array $episode_ids, int $current_year, bool $is_table ): array {
		...
		foreach ( $episode_ids as $episode_id ) {
			...
			$guestcast = $this->tvmaze_get( 'https://api.tvmaze.com/episodes/' . $episode_id . '/guestcast' );
...
Add to `$cli_loader` in `php/_components/class-wp-cli.php` (between `cli-calc.php` and `cli-check.php`):
...
**Quad-monthly catalog audit:**

1. `wp lwtv audit shows --format=csv > audit-$(date +%F).csv` (or letter-by-letter to spread the work across days/people: `--letter=a`, …, `--letter=num`, `--letter=intl`)
2. Import into Google Sheets. Everything in the file needs action — no "Keep" rows to filter. Show-level rows (empty `character` column) are status changes; character rows are missing-year additions. `edit_url` links straight to the right edit screen.
3. Animated/anime shows appear only as status rows; their characters are the manual fan-wiki pass, same as today.

**New show onboarding:**

`wp lwtv audit show <id> --recurring --all` for the historical pass. Add `--guests` only if the show is worth that level of scrutiny — guests rarely reappear.

## Known limitations (deliberate)

- **Animated/anime shows are status-only.** Voice actors play multiple characters; TVMaze cast data can't be mapped to LWTV characters. That check stays manual (fan wikis).
- **Catalog character check is a heuristic, not proof.** A regular written out mid-year without dying still gets flagged. Review list, not auto-fix — no `--fix` flag on purpose.
- **Deep-audit matching is fuzzy.** Normalized name matching misses "Dr." vs "Doctor" variance; sparse guestcast data on old episodes inflates "Verify" rows for early seasons.
- **`--all` on a soap is a first-run marathon** (10k+ episodes ≈ hours). The confirmation prompt exists for exactly this; caching makes rerun cost near zero.
- **Dead characters are included in `--all` deep audits** (their historical years are the point) but skipped everywhere else. Flip the `$is_dead && ! $do_all` condition if that's wrong.
- **`lezshows_char_list` can be stale** if a character was edited without triggering show recalculations; the REGEXP fallback covers the empty case. Run shadow sync / calc first if suspicious.
- **`get_tvmaze_info_show()` doesn't check status codes** — the audit compensates by validating the `id` key and retrying once. A proper fix inside the calendar class would benefit the calendar too.
