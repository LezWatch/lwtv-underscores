...
|---|---|
| `TMDB_Task`, `Cache_Queue`, `Calculation_Task`, `FixCharShows_Task`, `Facet_Reindex_Task`, `Statistics_Cache_Warming` | `TMDB_Batch_Task`, `Cache_Batch_Task`, `BYQ_Task`, `Imdb_Verify_Task`, `Watch_URLs_Task`, `Wikidata_Qid_Task` |

Code that runs on `save_post` must not assume the second set exists. `CPTs\Characters` checks `is_action_scheduler_available()` before scheduling `BYQ_Task::AS_INVALIDATE_HOOK`. The `Scheduler::queue_*()` helpers guard themselves:

| Helper | Without Action Scheduler |
|---|---|
| `queue_tmdb_batch()` | Schedules a per-post `TMDB_Task` through `schedule_task( 'tmdb', … )` (WP-Cron). |
| `cache_queue()` | `queue_cache_batch()` returns `false`, so it falls back to `Cache_Queue` (purge on shutdown). |
| `queue_imdb_verify()`, `queue_wikidata_qid()` | Return `false`; nothing is queued. The next save with Action Scheduler present, or the CLI backfills, pick the posts up. |

The `get_*_status()` methods return their usual shape with `next_scheduled` set to `false`.
...
Every task class hooks the generic name (for example `add_action( 'lwtv_calculation_task', ... )`), so both paths reach the same handler. Never schedule a per-post hook name: nothing listens on it.
...
1. `queue_post()` appends a post ID to a list stored in a non-autoloaded option (`_Helpers\Queue_Store`) and, if no run is pending, schedules one Action Scheduler action.
...
The queues are stores, so they live in options, not transients. See [caching.md](caching.md#cache-vs-store).

### Retry ceilings

A queue that reschedules itself every 60 seconds never drains on its own while anything is left in it. Without a ceiling, one post the remote end will never answer for keeps the whole queue alive and is re-requested roughly 1,400 times a day.

`Wikidata_Qid_Task` keeps a per-post failure count in a separate option (`ATTEMPTS`, `lwtv_wikidata_qid_attempts`) so the count survives the queue being rewritten each run:
