	...
	if ( '' !== $all_actors ) {
		foreach ( $all_actors as $each_actor ) {
			...
			if ( lwtv_plugin()->is_actor_queer( $each_actor ) ) {
				$the_actors[ $each_actor ]['queer'] = ' <span role="img" aria-label="Queer IRL Actor" data-bs-target="tooltip" title="Queer IRL Actor" class="character-cliche queer-irl">' . lwtv_plugin()->get_symbolicon( svg: 'rainbow.svg', icon: 'svg-cloud' ) . '</span>';
			}
		}
	} else {
		$the_actors[0] = array(
