			...
			<ul class="network-data list-group">
				...
				if ( isset( $next_ep['nextep'] ) ) {
					...
					if ( isset( $next_ep['summary'] ) && 'TBD' !== $next_ep['summary'] ) {
						...
						echo '<div class="collapse" id="episodeSummary"><div class="card card-body">' . esc_html( stripslashes( $next_ep['summary'] ) ) . '<br /><small><a href="' . esc_url( $tvmaze ) . '" target="_new">Powered by TVMaze</a></small></div></div>';
					} else {
						echo '</li>';
					}
				}
