		...
		if ( empty( $havecharcount ) || '0' === $havecharcount ) {
			...
			if ( isset( $chars_by_role['guest'] ) && is_array( $chars_by_role['guest'] ) ) {
				...
				foreach ( $chars_by_role['guest'] as $character ) {
					...
					$grave            = ( has_term( 'dead', 'lez_cliches', $character['id'] ) ) ? '<span role="img" aria-label="RIP Tombstone" title="RIP Tombstone" class="charlist-grave-sm">' . lwtv_plugin()->get_symbolicon( svg: 'rest-in-peace.svg', fontawesome: 'fa-times-circle', max_size: '15' ) . '</span>' : '';
					?>
