...
- Fixed show-ended tool to consider if we already gave it an end year.
...
- Corrected weird FacetWP quick where it wasn't syncing everyone properly.

### Added

- Add filter to re-sync characters in FacetWP when an actor name is changed.

## [7.2.1] - 2026-08-28

### Changed

- Character scoring now permanently uses the longevity-weighted model instead of the retired legacy one; no stored show scores changed as a result.
- `wp lwtv calc` no longer reports scoring feature-flag state, since those flags have been retired.

### Fixed

- Fixed `wp lwtv score-preview` fatally erroring on every run.
