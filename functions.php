...
/**
 * Get context-aware mood chips for the AI Discovery panel.
 *
 * @return array Array of prompt strings for quick-start chips.
 */
function lwtv_get_discovery_mood_chips() {
	$chips = array();
	...
	if ( empty( $chips ) ) {
		$chips = array(
			...
			__( 'Happy ending drama', 'lwtv-underscores' ),
			__( 'Worth watching British shows', 'lwtv-underscores' ),
			__( 'Ongoing shows with no dead queer characters', 'lwtv-underscores' ),
		);
	}

	return apply_filters( 'lwtv_discovery_mood_chips', $chips );
}
...
function lwtv_discovery_widget_footer() {
	...
	$mood_chips = lwtv_get_discovery_mood_chips();
